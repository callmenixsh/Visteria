...
async function ensureIndexes(collection) {
  await Promise.all([
    collection.createIndex({ siteId: 1, visitorHash: 1 }),
    collection.createIndex({ siteId: 1, lastSeenAt: -1 }),
    collection.createIndex({ 'visits.visitedAt': -1 }),
  ])
}
...
export async function getVisitsCollection() {
  ...
  ensureIndexes(cachedCollection).catch((error) => {
